...
public static class CosmicTables
{
    ...
    public class Mission_Table : Table<MissionInfo>, IDisposable
    {
        ...
        private readonly NameColumn _nameColumn = new() { Label = T("Name") };
        private readonly IdColumn _idColumn = new() { Label = T("ID") };
        private readonly JobColumn _jobColumn = new() { Label = T("Job") };
        private readonly MissionColumn _missionColumn = new() { Label = T("Rank") };
        private readonly CompletionColumn _completionColumn = new() { Label = T("Status") };
        private readonly ClassScoreColumn _classScoreColumn = new() { Label = T("Class") };
        private readonly CosmocreditColumn _cosmoColumn = new() { Label = T("Cosmo") };
        private readonly LunarCreditColumn _lunarColumn = new() { Label = T("Lunar") };
        private readonly DroneCreditColumn _droneColumn = new() { Label = T("Dronebits") };
        private readonly PlanetTokensColumn _planetTokenColumn = new() { Label = T("Mount") };
        private readonly SPMColumn _spmColumn = new() { Label = T("SPM") };
        private readonly TurninColumn _turninColumn = new() { Label = T("Goal") };
        private readonly PlanetColumn _planetColumn = new() { Label = T("Moons") };
        private readonly ProfileColumn _profileColumn = new() { Label = T("Profile") };
        private readonly NotesColumn _notesColumn = new() { Label = T("Notes") };
        private readonly AllRelicExpColum _allExpColumn = new() { Label = T("Exp") };

        public Mission_Table(List<MissionInfo> itemList) : base("Item_Table_V2", itemList)
        {
            _enabledColumn = new EnabledColumn(this) { Label = T("Enabled") };
            ...
            foreach (var (tier, flag) in tierFlags)
            {
                ...
                headers.Add(new RelicExpColumn(tier, flag) { Label = T("Exp {0}", tierName) });
            }
    ...
    {
        private readonly EnabledColumn_NF _enabledColumn = new() { Label = T("Enabled") };
        private readonly NameColumn _nameColumn = new() { Label = T("Name") };
        private readonly IdColumn_NF _idColumn = new() { Label = T("ID") };
        private readonly JobColumn_NF _jobColumn = new() { Label = T("Jobs") };
        private readonly MissionColumn_NF _missionColumn = new() { Label = T("Rank") };
        private readonly CompletionColumn_NF _completionColumn = new() { Label = T("Status") };
        private readonly TurninColumn_NF _turninColumn = new() { Label = T("Goal") };
        private readonly ProfileColumn_NF _profileColumn = new() { Label = T("Profile") };
        private readonly NotesColumn_NF _notesColumn = new() { Label = T("Notes") };
    ...
    private sealed class NameColumn : VerticalCenterColumnString
    {
        ...
        public override void DrawColumn(MissionInfo mission, int _)
        {
            ...
            if (mission.SheetInfo.Jobs.Contains(18) && !GatheringUtil.FishingPreset.ContainsKey(mission.Id))
            {
                ...
                {
                    ImGui.BeginTooltip();
                    ImGui.Text(T("This mission doesn't have a fishing preset yet, this is your warning for this."));
                    ImGui.EndTooltip();
                }
            }
    ...
    private sealed class TurninColumn : ItemFilterColumn
    {
        ...
        public override void DrawColumn(MissionInfo item, int idx)
        {
            ...
            else if (item.SheetInfo.IsMaster)
            {
                ...
                if (ImGui.Button(T("Master Settings")))
                {
                ...
                if (ImGui.BeginPopup(masterPopup))
                {
                    ...
                    if (C.MissionConfig.TryGetValue(item.Id, out var configInfo))
                    {
                        ...
                        {
                            ImGui.Separator();
                            if (ImGui.RadioButton(T("After X Crafts"), itemTurnin))
                            {
                            ...
                            if (ImGui.IsItemHovered())
                            {
                                ImGui.SetTooltip(T("Will turn in after X amount of crafts have been completed\n" +
    ...
    private sealed class ProfileColumn : ItemFilterColumn
    {
        ...
        public override void DrawColumn(MissionInfo item, int idx)
        {
            ...
            if (sheetInfo.Attributes.HasFlag(MissionAttributes.Craft))
            {
                ...
                if (ImGui_Ice.ImageButtonWithText(icon.GetWrapOrEmpty(), T("Craft Recipes"), $"{item.Id}_{item.SheetInfo.Name}_Craft", jobIconSize))
                {
            ...
            if (gatherProfile)
            {
                ...
                if (!collectable)
                {
                    ...
                    if (C.MissionConfig.TryGetValue(item.Id, out var config))
                    {
                        ...
                        if (ImGui_Ice.ImageButtonWithText(icon.GetWrapOrEmpty(), T("Gather Profile"), $"{item.Id}_{item.SheetInfo.Name}_BTN/MIN", jobIconSize, 0.5f, 2))
                        {
